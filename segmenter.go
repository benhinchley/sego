...
// LoadDictionaryFromReader loads a dictionary from the provided io.Reader
func (seg *Segmenter) LoadDictionaryFromReader(r io.Reader) {
	...
	// 逐行读入分词
	// Read segmentation line by line
	for {
		size, _ := fmt.Fscanln(reader, &text, &freqText, &pos)
		...
		if size == 0 {
			...
			// 没有词性标注时设为空字符串
			// set to an empty string without a part of speech tag
			pos = ""
		}
